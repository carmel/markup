...
func Build() {
	...
	filepath.Walk(sourcePath, func(path string, info os.FileInfo, err error) error {
		...
		if fileExt == ".md" {
			...
			slog.Info("Building", "link", article.Link)
			// Generate file path
			...
			articleInfo := ArticleInfo{
				...
				Date:       article.Time.Format(time.DateOnly),
				Title:      article.Title,
				Link:       article.Link,
				Top:        article.Top,
			}
	...
	if len(visibleArticles) == 0 {
		log.Fatalln("Must be have at least one article")
	}
	// Sort by date
	sort.Sort(articles)
	...
	for _, source := range srcList {
		if matches, err := filepath.Glob(filepath.Join(rootPath, source)); err == nil {
			for _, srcPath := range matches {
				slog.Info("Copying", "src", srcPath)
				file, err := os.Stat(srcPath)
				if err != nil {
					log.Fatalln("Not exist: " + srcPath)
				}
				fileName := file.Name()
