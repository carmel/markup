...
func ParseGlobalConfigByCli(c *cli.Context, develop bool) {
	...
	if globalConfig == nil {
		...
		if globalConfig == nil {
			log.Fatalln("Parse config.yml failed, please specify a valid path")
		}
	}
}
...
func New(c *cli.Context) {
	...
	if blogTitle == "" {
		if c.String("title") != "" {
			...
			log.Fatalln("Please specify the name of the blog post")
		}
	}
	...
	if c.Bool("post") && c.Bool("page") {
		log.Fatalln("The post and page arguments are mutually exclusive and cannot appear together")
	}
	if c.Bool("post") {
		postType = "post"
	}
	...
	if c.String("date") != "" {
		...
		if err != nil {
			log.Fatalln("Illegal date string")
		}
	} else {
...
func Convert(c *cli.Context) {
	...
	if args.Len() > 0 {
		...
		log.Fatalln("Please specify the posts source path")
	}
	if args.Len() > 1 {
	...
	if !Exists(sourcePath) || !Exists(rootPath) {
		log.Fatalln("Please specify valid path")
	}
